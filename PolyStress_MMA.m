...
%-------------------------------------------------------------------------%
%-------------------------------------------------------------------------%
% PolyStress_MMA.m: Code with MMA (MODIFIED FROM PolyStress.m)
...
%-------------------------------------------------------------------------%
function [z,V,fem] = PolyStress_MMA(fem,opt)
...
% while (Iter<opt.MaxIter)
  ...
  for j = 1:opt.MMA_Iter
      ...
      f0val = J; df0dx = dJdz(Eid);           df0dx2 = 0*df0dx;
      fval = 0;  dfdx = zeros(1,length(Eid)); dfdx2 = zeros(1,length(Eid));
      [znew, ~, ~, ~, ~, ~, ~, ~, ~, L(Eid),U(Eid)] = mmasub(...
          1, length(Eid), Iter, z(Eid), zmin, zmax, zold1(Eid),zold2(Eid), ...
          f0val, df0dx, df0dx2, fval, dfdx, dfdx2, ...
          L(Eid),U(Eid), a0, a_mma, cmma, d_mma);
