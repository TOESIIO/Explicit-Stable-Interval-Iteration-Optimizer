...
%-------------------------------------------------------------------------%
%-------------------------------------------------------------------------%
% PolyStress_GCMMA.m: Code with GCMMA (MODIFIED FROM PolyStress.m)
...
%-------------------------------------------------------------------------%
function [z,V,fem] = PolyStress_GCMMA(fem,opt)
...
a0 = 1; a_mma = 0; cmma= 1000; d_mma = 0;  zold1 = z; zold2 = z;
epsimin = 1e-7;  raa0 = 1e-4; raa = 1e-4; raa0eps = 1e-7; raaeps = 1e-7;
...
% while (Iter<opt.MaxIter)
  ...
  for j = 1:opt.MMA_Iter
      ...
      f0val = J; df0dx = dJdz(Eid);
      fval = 0;  dfdx = zeros(1,length(Eid)); 
      [L(Eid),U(Eid), raa0, raa] = asymp(Iter, length(Eid), z(Eid), zold1(Eid),zold2(Eid), ...
          zmin, zmax, L(Eid),U(Eid), raa0, raa, raa0eps, raaeps, df0dx, dfdx);
      [znew, ~, ~, ~, ~, ~, ~, ~, ~, ~, ~] = gcmmasub(...
          1, length(Eid), Iter, epsimin, z(Eid), zmin, zmax, L(Eid),U(Eid), ...
          raa0, raa, f0val, df0dx, fval, dfdx, a0, a_mma, cmma, d_mma);
      zold2(Eid)=zold1(Eid);  zold1(Eid)=z(Eid);
